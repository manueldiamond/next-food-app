...
const SearchFilters=({onSearch,defaultText,onFilter,selectedCategory}:searchFiltersProps)=>{
    ...
    <div className='w-full'>
        <form action={search} className=" container search__filters w-full flex gap-5">
            <div className={` ${active?" scale-105 shadow-[0px_4px_20px]":"shadow-sm"} transition-shadow search__bar my-1 text-[#3C2F2F] min-h-[35px] sm:min-h-[40px] md:min-h-[60px] bg-white flex gap-3 md:gap-5 rounded-[20px] flex-1 centered pl-5  shadow-black/20`}>
            ...
            <button className={ `${active?"w-[0px] opacity-0 !p-0 h-full":" h-full aspect-square "} !transition-all bg-accent rounded-[20px] shadow-xl centered `}>
