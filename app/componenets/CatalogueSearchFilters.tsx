...
const SearchFilters=()=>{
    ...
    <div className='w-full'>
        <div className=" container search__filters w-full flex gap-5">
            <div className=' search__bar my-1 text-[#3C2F2F] min-h-[60px] bg-white flex gap-5 rounded-[20px] flex-1 centered pl-5 shadow-[0px_4px_10px] shadow-black/20'>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor" className="w-6 h-6">
                    <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
                </svg>

                <input placeholder="Search" className='outline-none text-lg font-medium bg-transparent flex-1 h-full w-full'/>
            </div>
            <button className='bg-accent rounded-[20px] shadow-xl centered w-[60px] h-[60px]'>
       ...
       <ul className='py-8 container !max-w-max flex gap-3 overflow-x-scroll scroll-smooth scrollbar-hidden  h-max'>
       {list.map(item=>(
           <li onClick={()=>setSelectedCategory(item)} key={item} className={`${item===selectedCategory?" bg-accent text-[#F5F5F5] shadow-md":" font-medium text-base bg-[#F3F4F6] text-gray-2 cursor-pointer hover:text-accent"} rounded-[20px] px-6 py-3`}>
               {item}
           </li>
       ))}
       </ul>
